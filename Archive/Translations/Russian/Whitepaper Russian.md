...
Ссылка на Желтую бумагу Технические детали: [https://github.com/MorpheusAIs/Docs/blob/main/Yellowpaper-International/YellowPaperRussian.md](https://github.com/MorpheusAIs/Docs/blob/main/Translations/Russian/Yellowpaper%20Russian.md)
...
- 3 456 токенов для кода. Доказательство кода добавленного в репозиторий Morpheus та Github.
- 3 456 токенов для капитала. Доказательство вклада доходности stETH, 50% которого будут обменены на MOR, а остальная часть (50%) будет добавлена вместе с MOR в пул ликвидности в качестве ликвидности принадлежащей протоколу.
...
Капитал - ETH, заблокированные в смарт-контракте, который генерирует доходность (например, ETH в контракте stETH Lido Staking). 50% доходности stETH регулярно обменивается на MOR через децентрализованную биржу и вместе с другой частью 50% доходности stETH добавляется в пул ликвидности. Таким образом, поставщики капитала добавляют примерно 50% MOR и 50% stETH, со временем увеличивая ликвидность пула AMM MOR / stETH.
  ...
  - Капитал. Поставщики капитала получают MOR в зависимости от количества stETH внесенного в качестве капитала по отношению ко всем stETH внесенным в смарт-контракт, таким образом вознаграждения в MOR будут распределены пропорционально.
...
- Капитал. Плата за предоставление stETH ликвидности для токена MOR обеспечит большую часть вознаграждения.
...
С момента запуска Биткойна люди спорили о том, "что произойдет, когда вознаграждения за блоки наконец прекратятся?". Чтобы избежать этих бесполезных дебатов в контексте Morpheus и продолжить привлечение новых программистов, сообществ, вычислительных мощностей и поставщиков капитала надолго в будущее, мы предлагаем "остаточную эмиссию" токенов MOR. Она начнется после того, как будут выпущены последние токены MOR в день 5,833 по графику распределения. 
...
Поставщик капитала - это пользователь, который предоставляет stETH, который генерирует доходность для сети Morpheus, которая становится ликвидностью, принадлежащей протоколу (PoL). Смарт-контракт поставщика капитала будет предоставлять 50% от полученной доходности stETH для покупки MOR на децентрализованной бирже (AMM) и вместе с остальными 50% доходности stETH будет размещать их в виде ликвидности, что обеспечит ликвидность всем программистам, членам сообщества и поставщикам вычислений. Комиссии от обменов заработанные ликвидностью будут реинвестированы обратно.

Таким образом вся доходность stETH, полученная от депозитов, конвертируется в ликвидность, принадлежащую протоколу (PoL). Доходность остается в виде ликвидности навсегда, но вы можете вывести свои stETH в любое время после 7 дней первоначальной блокировки, необходимых для минимизации возможности манипулировани посредством снятия средств до момента когда доходность протоколом еще не получена. После снятия stETH, начислений наград в MOR прекращается.

В результате поставщик капитала будет получать токены MOR (начисляются каждый блок) пропорционально общей сумме внесенного им stETH, генерирующего доходность. Например, если есть 100 поставщиков капитала, которые внесли по 1 stETH в день запуска сети, то каждый из них получит 1% от 3 456 токенов MOR в этот день = 34,56 MOR. 
...
Документ детально поясняющий концепцию Машины технокапитала, доступен по [ссылке](https://github.com/MorpheusAIs/Docs/blob/main/Translations/Russian/Techno%20Capital%20Machine%20TCM%20Russian.md)
...
## Доказательство, регистрация и вознаграждение за вычислений: 
...
6) Маршрутизатор соединяет пользователя с провайдером через TCP/IP.
7) Пользователь отправляет запрос ([LLM],[запрос]) поставщику.
8) Поставщик вычисляет запрос, отправляет результат пользователю.
9) Пользователь сообщает успешные метрики (такие как время, количество полученных IPS, результат теста «Пройдено/Не пройдено») маршрутизатору.
10) Маршрутизатор дает указание контракту вычислений зачислить поставщику MOR, если работа была проведена удовлетворительно.
11) (Некоторое время спустя) Поставщик запрашивает оплату MOR из Контракта на вычисления, а контракт на вычисления отправляет платеж MOR, если он действителен.
...
В течение первого года, следующего за периодом инициализации контракта поставщиков капитала, топ-100 поставщиков вычислений могут иметь право на пропорциональную долю в размере 2.4% от эмиссии MOR, которая рассчитывается маршрутизатором и учитывается в контракте вычислений.

Дополнительные подробности доступны в [Документе модели вычислений Yellowstone](https://github.com/MorpheusAIs/Docs/blob/main/Translations/Russian/Yellowstone%20Compute%20Model%20Russian.md)
...
Более развернутое описание работы защитного фонда доступно по [ссылке](https://github.com/MorpheusAIs/Docs/blob/main/Translations/Russian/Protection%20Fund%20Details%20Russian.md)
